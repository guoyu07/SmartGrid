(function($) {
    ...
    var selectedCheckRows, methods, loadElement, inputFocus,
        pfsXHR={},
        gridSettings=[];
    ...
    methods = {
        init: function(options) {
            var settings=$.extend({
                ...
                enableHistory: false,
                dataLinkAjax: [
                    'a[data-pfs-ajax-link]'
                ],
                dataFormAjax: [
                    'form[data-pfs-ajax-pagination]',
                    'form[data-pfs-ajax-filter]'
                ],
            ...
            this.each(function() {
                ...
                if (settings.ajaxUpdate.length > 0 || settings.ajaxUpdate === true) {
                    var eventCb = function(e) {
                        ...
                        var data = $(this).prop('tagName').toLowerCase() === 'form' ? $(this).serialize() : undefined;
        ...
        redrawTable: function() {
            return this.each(function() {
                ...
                elemTableBody.each(function(index, el) {
                    ...
                    var lastPadding = (elemTableBody.length - 1 == index ? scrollBarWidth : 0);
                    var lastHeader = $tableHeader.find('th[data-pfs-title-position="' + (index + 1) + '"]').innerWidth(width + lastPadding);
                    var lastFooter = $tableFooter.find('th[data-pfs-title-position="' + (index + 1) + '"]').innerWidth(width + lastPadding);
                });
